...
public:
  ...
  auto audio_start(uint32_t sample_rate) const -> std::expected<bool, NeoVIMICErrType>;
  auto audio_stop() const -> std::expected<bool, NeoVIMICErrType>;

  auto gps_close() const -> std::expected<bool, NeoVIMICErrType>;
  auto gps_has_lock() const -> std::expected<bool, NeoVIMICErrType>;
  auto gps_info() const -> std::expected<bool, NeoVIMICErrType>;
  auto gps_is_open() const -> std::expected<bool, NeoVIMICErrType>;
  auto gps_open() const -> std::expected<bool, NeoVIMICErrType>;

  auto io_button_is_pressed() const -> std::expected<bool, NeoVIMICErrType>;
  auto io_buzzer_enable() const -> std::expected<bool, NeoVIMICErrType>;
  auto io_buzzer_is_enabled() const -> std::expected<bool, NeoVIMICErrType>;
  auto io_close() const -> std::expected<bool, NeoVIMICErrType>;
  auto io_gpsled_enable() const -> std::expected<bool, NeoVIMICErrType>;
  auto io_gpsled_is_enabled() const -> std::expected<bool, NeoVIMICErrType>;
  auto io_is_open() const -> std::expected<bool, NeoVIMICErrType>;
  auto io_open() const -> std::expected<bool, NeoVIMICErrType>;
